...
class About extends CI_Controller {
	...
	private $tbl = 'dtb_wahyudin_about';
	...
	public function index(){
		...
		$data = [
			...
			'content' => $this->load->view('back/about/index' ,'',true),
		];
	...
	function get_data_about(){
		...
		$where_like['data'][] = array(
			'column' => 'id,picture,header,description,foot_note',
		...
		if ($query<>false) {
			...
			foreach ($query->result() as $val) {
				if ($val->id>0) {
            		...
            		$img = "<img src='".base_url()."assets/picture/pic_about/".$val->picture."' style='height:80px;width:80px;'>";
	...
	function save_about($id=null){
		if (!$this->input->is_ajax_request()) {
			exit('No direct script access allowed');
		}

		$config['upload_path']          = './assets/picture/pic_about/';
		...
		if ($id != null && $id > 0) {
			...
			$old_path = 'assets/picture/pic_about/'.$old_img;
